...
namespace Riptide
{
    ...
    {
        #region ClientInfo
        /// <inheritdoc cref="AddClientInfo(Message, ClientInfo)"/>
        /// <remarks>This method is simply an alternative way of calling <see cref="AddClientInfo(Message, ClientInfo)"/>.</remarks>
        public static Message Add(this Message message, ClientInfo value) => AddClientInfo(message, value);

        /// <summary>Adds <see cref="ClientInfo"/> to the message.</summary>
        /// <param name="value">The <see cref="ClientInfo"/> to add.</param>
        /// <returns>The message that <see cref="ClientInfo"/> was added to.</returns>
        public static Message AddClientInfo(this Message message, ClientInfo value)
        {
            return message.AddString(value.name).AddInts(value.cards.ToArray());
        }

        /// <summary>Retrieves <see cref="ClientInfo"/> from the message.</summary>
        /// <returns>The <see cref="ClientInfo"/> that was retrieved.</returns>
        public static ClientInfo GetClientInfo(this Message message)
        {
            return new ClientInfo(message.GetString(), message.GetInts().ToList<int>());
        }
        #endregion
